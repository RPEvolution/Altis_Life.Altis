...
*/
private["_dialog","_inv","_lic","_licenses","_near","_near_units","_mstatus","_shrt","_side","_picture","_tooltip"];
...
_mstatus ctrlSetStructuredText parseText format["<img size='1.3' image='textures\icons\bank.paa'/> <t size='0.8px'>$%1</t><br/><img size='1.2' image='textures\icons\money.paa'/> <t size='0.8'>$%2</t>",[life_atmcash] call life_fnc_numberText,[life_cash] call life_fnc_numberText];
ctrlSetText[2009,format["Gewicht: %1 / %2", life_carryWeight, life_maxWeight]];
{
	_str = [_x] call life_fnc_varToStr;
	_shrt = [_x,1] call life_fnc_varHandle;
	_picture = [_x] call life_fnc_handleItemPicture;
	_tooltip = [_x] call life_fnc_handleItemTooltip;
	...
	if(_val > 0) then
	{
		...
		_inv lbSetPicture[(lbSize _inv)-1,_picture];
