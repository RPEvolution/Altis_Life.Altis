...
while{_settingup} do
{
	switch(true) do
	{
		// Police and Altis Service can both CheckRegistration
		case ((playerSide == independent || playerSide == west) && !(ctrlShown _Btn4)): {
		...
		case ((playerSide == west) && !(ctrlShown _Btn5)): {
		...
		// Altis Service and Admins can TowVehicles and UnflipVehicles
		case ((playerSide == independent || (__GETC__(life_adminlevel) > 0)) && !(ctrlShown _Btn5)): {
			_Btn5 ctrlSetText localize "STR_vInAct_TowVehicle";
			_Btn5 buttonSetAction "[life_vInact_curTarget] spawn life_fnc_adacImpound;";
		...
		// Everyone can RepairVehicles, FillVehicles and GetInVehicles
		default {
			//Set Repair Action
			...
			//Set Fill Action
			_Btn2 ctrlSetText localize "STR_vInAct_Fill";
			_Btn2 buttonSetAction "[life_vInact_curTarget] spawn life_fnc_fillAction;";
			
			if(count (nearestObjects [player, ["Land_fs_feed_F"], 25]) > 0) then {_Btn2 ctrlEnable true;} else {_Btn2 ctrlEnable false;};
...
/*
		...
		if(_curTarget isKindOf "Ship") then {
			...
			if(typeOf (_curTarget) in ["C_Kart_01_Blu_F","C_Kart_01_Red_F","C_Kart_01_Fuel_F","C_Kart_01_Vrana_F","O_Heli_Transport_04_bench_f","O_Heli_Transport_04_F","B_Heli_Transport_03_unarmed_F"]) then {
			...
			/*
			if(typeOf (_curTarget) in ["B_Heli_Transport_03_unarmed_F","O_Heli_Transport_04_F","O_Heli_Transport_04_medevac_F","O_Heli_Transport_04_repair_F"]) then {
	...
	if(_curTarget isKindOf "Ship") then {
		...
		if(typeOf (_curTarget) in ["C_Kart_01_Blu_F","C_Kart_01_Red_F","C_Kart_01_Fuel_F","C_Kart_01_Vrana_F","B_Heli_Transport_03_unarmed_F","O_Heli_Transport_04_F","O_Heli_Transport_04_medevac_F","O_Heli_Transport_04_repair_F"]) then {
	...
	if(((["adac"] call life_fnc_permLevel) > 0) || (__GETC__(life_adminlevel) > 1)) then {
